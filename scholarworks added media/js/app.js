...
(function() {
  ...
  function initMobileSelects() {
    if (!isMobile()) {
      // Remove custom dropdowns on desktop
      document.querySelectorAll('.mobile-select-wrap').forEach(function(wrap) {
        var sel = wrap.querySelector('select.fs');
        if (sel) {
    ...
    // Only target assignment type selects in .hmf (mobile inline form)
    var forms = document.querySelectorAll('.hmf');
    forms.forEach(function(form) {
      var fgs = form.querySelectorAll('.fg');
      fgs.forEach(function(fg) {
        var label = fg.querySelector('.fl');
        if (!label || label.textContent.trim() !== 'Assignment type') return;
        var sel = fg.querySelector('select.fs');
        if (!sel || fg.querySelector('.mobile-select-wrap')) return;
        ...
        Array.from(sel.options).forEach(function(opt, i) {
          ...
          item.addEventListener('click', function() {
            ...
            list.querySelectorAll('.mobile-select-item').forEach(function(it) { it.classList.remove('selected'); });
        ...
        trigger.addEventListener('click', function(e) {
          ...
          document.querySelectorAll('.mobile-select-list.open').forEach(function(l) { l.classList.remove('open'); });
  ...
  document.addEventListener('DOMContentLoaded', initMobileSelects);
  var resizeTimer;
  window.addEventListener('resize', function() {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(initMobileSelects, 200);
  });
})();
